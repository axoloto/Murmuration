...
class Packet
{
  ...
  Packet& openBundle(uint64_t time)
  {
    if (m_inBundle > 0)
    {
      ...
      const int32_t offset = m_sizePosB == nullptr ? 0 : ptrDiff(m_sizePosB, m_args.begin());
...
template <>
inline Packet& Packet::put<int32_t>(int32_t x)
{
  return int32(x);
}
template <>
inline Packet& Packet::put<float>(float x)
{
  return float32(x);
}
template <>
inline Packet& Packet::put<const char*>(const char* x)
{
  return string(x);
}
template <>
inline Packet& Packet::put<Blob>(Blob x)
{
  return blob(x);
}
