...
inline uint32_t convert32(uint32_t)
{
  throw std::logic_error("Invalid byte order");
}

template <>
inline uint32_t convert32<NetworkByteOrder>(uint32_t x)
{
...
template <>
inline uint32_t convert32<HostByteOrder>(uint32_t x)
{
  return x;
}

template <ByteOrder B>
inline uint64_t convert64(uint64_t)
{
  throw std::logic_error("Invalid byte order");
}

template <>
inline uint64_t convert64<NetworkByteOrder>(uint64_t x)
{
#if defined(OSCPP_LITTLE_ENDIAN)
  return bswap64(x);
#else
...
template <>
inline uint64_t convert64<HostByteOrder>(uint64_t x)
{
  return x;
}
} // namespace OSCPP
